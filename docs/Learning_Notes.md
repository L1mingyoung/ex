  ...
  postgres:
    ...
    healthcheck: # 健康检查：Docker 定期执行这个命令判断服务是否正常
      test:
        [
          'CMD-SHELL',
          'pg_isready -U ${DB_USER:-postgres} -d ${DB_NAME:-companion}',
        ]
...
## 踩坑补充：Docker 密码与 Python 服务启动

### Docker 容器密码踩坑

**现象：** NestJS 启动时报 `password authentication failed for user "postgres"`

**原因：** Docker 容器启动时通过 `POSTGRES_PASSWORD` 环境变量设置数据库密码，`.env` 中的 `DB_PASSWORD` 必须和它完全一致。

**排查方法：**

```bash
# 查看容器实际配置的密码
docker inspect companion-postgres --format '{{range .Config.Env}}{{println .}}{{end}}' | findstr POSTGRES
```
...
POSTGRES_PASSWORD=change-me   ← 容器实际密码
...
```bash
docker exec -it companion-postgres psql -U postgres -c "ALTER USER postgres PASSWORD 'postgres';"
```

`ALTER USER` 直接修改数据库内部密码，立即生效，无需重启容器。
...
- `POSTGRES_PASSWORD` 只在容器**首次初始化**时生效，后续改环境变量没用
- 密码存储在数据库内部，不是配置文件，所以 `ALTER USER` 是最直接的修改方式
- 开发时建议统一改成简单密码（如 `postgres`），方便记忆
...
`main.py` 中写了两种 mock 判断方式：

```python
USE_MOCK = "--mock" in sys.argv or os.environ.get("MOCK_EMBEDDING") == "1"
```

但 `uvicorn` 不支持传递自定义参数给应用：

```bash
# ❌ 报错：No such option '--mock'
uv run uvicorn main:app --port 8000 --mock

# ❌ 报错：Got unexpected extra argument (--mock)
uv run uvicorn main:app --port 8000 -- --mock
```

**结论：** 用 `uvicorn` 启动时，自定义参数只能走**环境变量**，不能用命令行参数。

**完整流程：**

```bash
cd d:\Code\AI\companion\python
uv sync                              # 安装依赖
MOCK_EMBEDDING=1 uv run uvicorn main:app --port 8000   # Mock 模式启动
```

启动后验证：访问 http://localhost:8000/health
...
| 方式                  | 是否需要公网地址 | 说明                                                       |
| --------------------- | ---------------- | ---------------------------------------------------------- |
| **WebSocket**（采用） | 不需要           | 主动连接 QQ 网关，出站连接即可，本地也能开发               |
| **Webhook**           | 需要             | QQ 平台推送事件到你的服务器，必须有公网 URL + 域名 + HTTPS |

**结论：** 选择 WebSocket 模式，云服务器部署时无需配置域名和反向代理。
...
| 方式         | 格式                          | 状态          |
| ------------ | ----------------------------- | ------------- |
| Token 鉴权   | `QQBot {AppID}.{Token}`       | ❌ **已废弃** |
| Access Token | `QQBot {AppID}.{AccessToken}` | ✅ 当前推荐   |

**获取 Access Token：**

```javascript
// POST https://bots.qq.com/app/getAppAccessToken
const body = { appId: '你的AppID', clientSecret: '你的AppSecret' };
// 返回 { access_token, expires_in }
```

Token 有效期 72 小时，代码中提前 1 小时刷新，避免过期导致断连。

### 三、WebSocket 协议核心要点

```
连接 → Op 10 Hello（心跳间隔）→ Op 2 Identify（鉴权）→ READY
                                     ↕
心跳：Op 1（上行） / Op 11 ACK（下行）
                                     ↕
消息：Op 0 Dispatch（t = 事件类型, d = 数据）
```

**关键 Op 码：**

- **Op 10**：Hello，含心跳间隔（毫秒）
- **Op 2**：Identify，鉴权请求
...
- WebSocket 断连后，如果重新鉴权（Op 2），会丢失断连期间的消息
- Op 6 Resume 让平台补发遗漏事件，保证消息不丢
...
fs.writeFileSync(
  ...
  JSON.stringify({
    seq, // 最后收到的事件序列号
    sessionId, // QQ 平台返回的 session_id
    timestamp, // 保存时间（超过 30 分钟不 Resume）
  }),
);
...
- `GROUP_AT_MESSAGE_CREATE`：群聊 @机器人 的消息

**发送回复 API：**

| 场景 | API 路径                             |
| ---- | ------------------------------------ |
| 私聊 | `POST /v2/users/{userId}/messages`   |
| 群聊 | `POST /v2/groups/{groupId}/messages` |
...
QQ 平台可能重复推送同一消息，用 `Set` 记录已处理的消息 ID：
...
### 七、start.bat 集成 QQ Bot

`start.bat` 通过检测 `.env` 文件中是否存在 `QQ_BOT_APP_ID` 来自动决定是否启动 QQ Bot：
...
- batch 脚本不会自动加载 `.env` 文件，需要用 `findstr` 检测
- QQ Bot 适配器内部用 `dotenv` 加载 `.env`，所以运行时能读到环境变量

### 八、沙箱 vs 正式环境

| 环境 | 网关地址                                    | 环境变量                    |
| ---- | ------------------------------------------- | --------------------------- |
| 沙箱 | `wss://sandbox.api.sgroup.qq.com/websocket` | `QQ_BOT_SANDBOX=1`          |
| 正式 | `wss://api.sgroup.qq.com/websocket`         | 不设置或 `QQ_BOT_SANDBOX=0` |

**开发建议：** 先在沙箱环境测试通过，审核通过后再切换到正式环境，只需改一个环境变量。

### 九、关键文件

| 文件                                 | 说明                             |
| ------------------------------------ | -------------------------------- |
| `adapters/qq-bot/index.js`           | QQ Bot 适配器 v2                 |
