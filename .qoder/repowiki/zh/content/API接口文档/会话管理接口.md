...
## 更新摘要
**所做更改**
- 新增QQ机器人适配器集成章节，详细说明跨平台会话连续性功能
- 更新会话管理API架构图，反映QQ机器人适配器的集成
- 新增会话映射机制和状态持久化功能说明
- 补充跨平台消息去重和频率限制策略
- 增加会话重建和断线恢复机制说明
...
## 简介
本文件为"会话管理接口"的完整API文档，覆盖会话实体的数据结构、CRUD接口设计、状态与清理策略、导入导出与迁移建议，以及与消息查询、聊天交互的关联关系。特别地，文档现已更新以支持新的QQ机器人适配器集成，实现了跨平台会话连续性功能，允许用户在Web端和QQ机器人之间无缝切换聊天体验。

## 项目结构
会话管理位于后端 NestJS 应用的 sessions 子系统，核心由实体、控制器、服务与模块组成，并通过聊天与消息模块进行业务联动。新增的QQ机器人适配器通过HTTP API与会话管理接口集成，实现了跨平台会话映射和状态同步。
...
subgraph "QQ机器人适配器"
QI["QQ Bot 适配器<br/>index.js"]
QA["适配器占位<br/>adapter.js"]
end
C --> S
S --> E
M --> C
...
- [chat.service.ts:1-200](file://src/chat/chat.service.ts#L1-L200)
...
- QQ机器人适配器：通过HTTP API与会话管理接口集成，实现跨平台会话映射和状态同步。

**章节来源**
- [session.entity.ts:32-63](file://src/sessions/entities/session.entity.ts#L32-L63)
- [sessions.controller.ts:4-34](file://src/sessions/sessions.controller.ts#L4-L34)
...
- [index.js:258-284](file://adapters/qq-bot/index.js#L258-L284)

## 架构总览
会话管理与聊天、消息模块存在紧密耦合：聊天服务在每次对话后会更新会话的消息计数与摘要；消息控制器提供按会话查询历史的能力；会话控制器提供基础的CRUD接口。QQ机器人适配器通过HTTP API与这些组件集成，实现了跨平台会话连续性。

```mermaid
sequenceDiagram
participant Client as "客户端"
participant QQBot as "QQ机器人适配器"
...
QQBot->>SessSvc : "getOrCreateSession(sessionKey)"
SessSvc->>SessRepo : "查询角色会话"
QQBot->>ChatCtrl : "POST /api/chat/ : sessionId"
ChatCtrl->>ChatSvc : "handleMessage(sessionId, content)"
```
...
**章节来源**
- [sessions.controller.ts:8-33](file://src/sessions/sessions.controller.ts#L8-L33)
...
### 跨平台会话连续性
QQ机器人适配器通过HTTP API与会话管理接口深度集成，实现了跨平台会话连续性功能。适配器能够识别用户身份并复用现有的会话，确保用户在Web端和QQ机器人之间的聊天记录保持一致。

### 会话映射机制
适配器维护了一个会话映射表，将QQ用户标识映射到系统会话ID：

- 私聊映射：`c2c-${qqUserId}` → sessionId
- 群聊映射：`group-${groupId}-${qqUserId}` → sessionId
- 角色维度：每个角色（characterId）对应一组会话

```mermaid
flowchart LR
User["QQ用户"] --> Key["会话键<br/>c2c-${userId}<br/>group-${groupId}-${userId}"]
Key --> Map["会话映射表<br/>sessionMap"]
Map --> Session["系统会话ID"]
...
- [index.js:259-284](file://adapters/qq-bot/index.js#L259-L284)

### 会话重建与断线恢复
适配器实现了智能的会话重建和断线恢复机制：

- 会话查询：启动时查询系统中是否存在同角色的现有会话
- 自动创建：如无现有会话，则创建新的会话并标记为QQ专用
- 断线恢复：通过WebSocket Op 6 Resume协议实现消息不丢失
- 状态持久化：会话映射和WebSocket状态保存到本地文件

### 消息去重与频率限制
为确保消息质量和用户体验，适配器实现了以下机制：

- 消息去重：使用Set记录已处理的消息ID，防止重复处理
- 频率限制：QQ被动回复限制为5分钟内最多2条
- 长消息分段：自动将超过2000字符的消息拆分为多段发送

### 状态持久化与恢复
适配器通过状态文件实现断线后的快速恢复：

- 状态文件：`.qq-bot-state.json`，包含seq、sessionId、sessionMap等信息
- 恢复条件：状态文件存在且时间戳在30分钟以内
- 会话映射恢复：恢复所有用户到会话ID的映射关系

**章节来源**
- [index.js:258-301](file://adapters/qq-bot/index.js#L258-L301)
- [index.js:408-406](file://adapters/qq-bot/index.js#L408-L406)
- [index.js:368-406](file://adapters/qq-bot/index.js#L368-L406)
- [index.js:104-131](file://adapters/qq-bot/index.js#L104-L131)
- [index.js:309-357](file://adapters/qq-bot/index.js#L309-L357)
...
- QQ适配器依赖：QQ机器人适配器依赖HTTP API与会话管理接口交互
...
+chat(sessionKey, sessionId, content)
+sendC2CReply(userId, content, msgId)
+sendGroupReply(groupId, content, msgId)
}
SessionsController --> SessionsService : "依赖"
SessionsService --> Session : "操作"
QQBotAdapter --> SessionsController : "HTTP API"
QQBotAdapter --> ChatController : "HTTP API"
```
...
- QQ适配器优化：会话映射内存存储，查询O(1)复杂度；状态文件定期持久化，平衡性能与可靠性
...
- QQ适配器问题
  - 现象：会话无法复用或频繁重建
  - 处理：检查角色ID配置、会话映射状态文件、网络连接状态

**章节来源**
- [sessions.service.ts:22-28](file://src/sessions/sessions.service.ts#L22-L28)
- [sessions.controller.ts:8-11](file://src/sessions/sessions.controller.ts#L8-L11)
- [index.js:292-301](file://adapters/qq-bot/index.js#L292-L301)

## 结论
会话管理接口围绕Session实体提供了基础的CRUD能力，并与聊天、消息模块形成清晰的职责分工。通过消息计数与滚动摘要机制，系统能在保证性能的同时维持高质量的上下文。QQ机器人适配器的集成进一步增强了系统的跨平台能力，实现了Web端与QQ机器人的会话连续性。建议后续补充参数校验、导出端点与数据库级级联策略，以进一步提升可用性与可靠性。
...
**章节来源**
- [sessions.controller.ts:20-23](file://src/sessions/sessions.controller.ts#L20-L23)
...
- 环境变量配置
  ...
  - QQ_BOT_APP_SECRET：QQ机器人应用密钥
  - QQ_CHARACTER_ID：关联的角色ID（默认"xiaoya"）
  - QQ_BOT_SANDBOX：沙箱模式开关（1启用，0禁用）
  - QQ_BOT_STATE_DIR：状态文件存储目录
- API基础配置
  - API_BASE：NestJS服务地址，默认"http://localhost:3000"
  - 网关地址：根据沙箱模式自动选择正式或沙箱网关

**章节来源**
- [index.js:38-58](file://adapters/qq-bot/index.js#L38-L58)
- [index.js:641-664](file://adapters/qq-bot/index.js#L641-L664)