...
- **QQ机器人适配器的增强功能**：统一API请求、智能消息分割、改进的会话管理、消息去重与频率限制、断线恢复机制
...
QQ["QQ 机器人适配器 v2<br/>adapters/qq-bot/index.js"]
...
- [shared/types.ts:1-167](file://shared/types.ts#L1-L167)
...
- **平台适配器**：
  ...
  - **QQ 机器人 v2**：WebSocket 接入 QQ Bot 网关，事件分发、会话映射、HTTP 回复，具备统一API请求、智能消息分割、消息去重与频率限制等增强功能。
...
participant QQ as "QQ 机器人适配器 v2"
...
**更新** 本节详细介绍了QQ机器人适配器的重大增强功能，包括统一API请求、智能消息分割、改进的会话管理、消息去重与频率限制等新特性。

- **设计要点**
  - 通过 WebSocket 与 QQ 官方网关建立长连接，接收事件（如 C2C_MESSAGE_CREATE、GROUP_AT_MESSAGE_CREATE）。
  - **统一API请求**：新增`qqApiRequest`函数，专门处理QQ官方API的统一请求，支持Access Token认证。
  - **智能消息分割**：新增`splitMessage`函数，支持2000字符限制的智能分割，按换行符断开，避免切割中文字符。
  - **消息去重与频率限制**：新增消息去重机制（Set存储已处理消息ID）和被动回复频率限制（每5分钟最多2条）。
  - **断线恢复**：支持Op 6 Resume机制，连接断开后可恢复之前的会话状态。
  - **改进的会话管理**：支持私聊和群聊的不同会话映射策略，私聊按用户维度，群聊按群+用户维度隔离。
  - **事件处理**：收到消息后，映射或创建会话，调用后端 /api/chat/{sessionId} 获取回复，再通过 HTTP API 回复用户。
  - **错误恢复**：连接断开后定时重连，心跳维持，异常日志输出。
- **关键流程**
  - **Access Token管理**：自动获取和刷新Access Token，支持提前刷新机制。
  - **鉴权与心跳**：收到 Hello 后启动心跳并发送 Identify，支持断线恢复。
  - **事件处理**：过滤自身消息，获取或创建会话，调用 chat 接口，发送回复。
  - **会话映射**：维护 qqUserId → sessionId 的内存映射，首次对话自动创建会话。
  - **长消息处理**：自动检测并分割超过2000字符的消息，逐段发送并记录回复次数。

```mermaid
sequenceDiagram
participant QQWS as "QQ WebSocket"
participant Adapter as "QQ 适配器 v2"
participant API as "后端控制器"
...
API-->>Adapter : "reply"
Adapter->>Reply : "HTTP POST 回复用户支持分段"
Reply-->>QQWS : "发送成功"
```
...
- **QQ 机器人 v2**：WebSocket 长连接与心跳维持，注意网络抖动下的重连策略与事件去重。**新增**：Access Token 自动刷新机制减少鉴权开销；智能消息分割避免单次请求过大；消息去重机制防止重复处理；频率限制控制避免触发平台限流。
- **后端流式响应**：SSE 需禁用代理缓冲，确保实时推送；大文本分块传输时注意编码与边界处理。
- **会话映射**：当前为内存 Map，生产环境建议持久化或使用分布式缓存，避免进程重启丢失。

**更新** 新增了QQ机器人适配器v2的性能优化考虑，包括Access Token管理、消息分割、去重和频率限制等机制。
...
- **QQ 机器人 v2**
  ...
  - **新增**：检查Access Token获取是否成功；确认消息去重机制是否正常工作；验证智能消息分割是否按预期执行
  - 日志：关注连接断开与重连日志、消息解析错误、HTTP 回复失败、Access Token刷新日志
- **后端**
  - 现象：SSE 无数据或提前关闭
  - 排查：SSE 响应头设置、代理缓冲、订阅完成信号与错误处理

**更新** 新增了QQ机器人适配器v2的故障排查指导，包括Access Token、消息去重、智能分割等相关问题的排查方法。
...
## 结论
该多平台适配器系统通过统一接口与共享类型，将平台差异隔离在网络层，使业务逻辑与 UI/SDK 层解耦。微信小程序与 uni-app 在流式能力受限时采用降级策略，QQ 机器人通过 WebSocket 与 HTTP 双通道实现事件驱动与消息回复。**最新版本的QQ机器人适配器v2**引入了统一API请求、智能消息分割、消息去重与频率限制、断线恢复等增强功能，显著提升了系统的稳定性、可靠性和用户体验。整体架构清晰、扩展性强，适合进一步接入 Telegram 等其他平台。

**更新** 结论部分强调了QQ机器人适配器v2的增强功能及其对整体系统的影响。
...
**新增** 本节详细介绍QQ机器人适配器v2的核心增强功能。

#### 统一API请求机制
- **qqApiRequest函数**：专门用于QQ官方API的统一请求，自动处理Access Token认证
- **支持的API**：私聊消息发送（/v2/users/{userId}/messages）、群聊消息发送（/v2/groups/{groupId}/messages）
- **错误处理**：统一的超时处理（10秒）和错误捕获机制

#### 智能消息分割算法
- **长度限制**：QQ单条消息最大2000字符
- **分割策略**：优先在换行符处分割，避免在中文字符中间截断
- **分段发送**：多段消息之间添加短暂延迟（500ms）避免触发限流
- **消息ID传递**：每段消息都携带原始消息ID，确保回复关联性

#### 消息去重与频率控制
- **去重机制**：使用Set存储已处理的消息ID，防止重复处理
- **容量管理**：最大存储1000条消息ID，超出时自动清理最旧记录
- **频率限制**：每5分钟内对同一消息最多回复2次，避免刷屏
- **自动清理**：频率统计在5分钟后自动清理，释放内存

#### 断线恢复与状态持久化
- **Resume机制**：支持Op 6 Resume协议，断线后可恢复之前的会话状态
- **状态保存**：将seq、sessionId和时间戳保存到本地文件，支持重启后恢复
- **超时检查**：超过30分钟的状态会被忽略，避免使用过期状态
- **错误处理**：无效会话时自动清理状态并重新连接

#### 会话管理增强
- **私聊会话**：按用户ID维度管理，每个用户独立会话
- **群聊会话**：按群ID+用户ID维度管理，同一群内不同用户有独立对话
- **自动创建**：首次对话时自动创建会话并存储映射关系
- **会话隔离**：群聊场景下确保不同用户的对话相互独立

**章节来源**
- [adapters/qq-bot/index.js:206-241](file://adapters/qq-bot/index.js#L206-L241)
- [adapters/qq-bot/index.js:270-316](file://adapters/qq-bot/index.js#L270-L316)
- [adapters/qq-bot/index.js:98-129](file://adapters/qq-bot/index.js#L98-L129)
- [adapters/qq-bot/index.js:359-357](file://adapters/qq-bot/index.js#L359-L357)
