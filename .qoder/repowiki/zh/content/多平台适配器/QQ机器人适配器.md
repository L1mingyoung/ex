...
## 更新摘要
**变更内容**
- 新增统一QQ API请求函数(qqApiRequest)，提供标准化的QQ平台API调用能力
- 实现智能长消息分割功能，支持2000字符限制的自动分段发送
- 改进会话管理，区分私聊和群聊上下文，实现群内@机器人精准响应
- 扩展事件意图覆盖，支持GROUP_AT_MESSAGE_CREATE和C2C_MESSAGE_CREATE事件
- 增强群聊@机器人处理逻辑，自动过滤mention标记并提取真实消息内容
- 实现Access Token鉴权机制，替代旧版Token方式
- 新增Op 6 Resume断线恢复功能，确保消息不丢失
- 实现消息去重和频率限制双重保护机制
- 增加状态持久化功能，支持断线后自动恢复
...
## 简介
本文档面向"QQ机器人适配器"的技术实现，聚焦于v2.0版本的重大升级特性，包括生产级适配器架构、Access Token鉴权机制、Op 6 Resume断线恢复、群聊支持、消息去重、频率限制、状态持久化等核心功能。文档详细阐述adapter.js与index.js的职责分工与协作关系，并提供针对QQ机器人特有消息类型的处理方案。
...
此外，与后端服务交互的关键接口定义在NestJS控制器与共享类型中。
...
**图表来源**
- [index.js:1-664](file://adapters/qq-bot/index.js#L1-L664)
- [adapter.js:1-35](file://adapters/qq-bot/adapter.js#L1-L35)
- [sessions.controller.ts:1-35](file://src/sessions/sessions.controller.ts#L1-L35)
...
- **统一QQ API请求**：新增qqApiRequest函数，提供标准化的QQ平台API调用
- **智能长消息分割**：实现2000字符限制的自动分段发送功能
...
- [index.js:67-84](file://adapters/qq-bot/index.js#L67-L84)
- [index.js:55-58](file://adapters/qq-bot/index.js#L55-L58)
- [index.js:408-406](file://adapters/qq-bot/index.js#L408-L406)
- [index.js:95](file://adapters/qq-bot/index.js#L95)
- [index.js:104-114](file://adapters/qq-bot/index.js#L104-L114)
- [index.js:122-131](file://adapters/qq-bot/index.js#L122-L131)
- [index.js:171-210](file://adapters/qq-bot/index.js#L171-L210)
- [index.js:212-252](file://adapters/qq-bot/index.js#L212-L252)
- [index.js:309-326](file://adapters/qq-bot/index.js#L309-L326)
- [index.js:328-353](file://adapters/qq-bot/index.js#L328-L353)
- [index.js:436-549](file://adapters/qq-bot/index.js#L436-L549)
...
- **频率限制**：限制被动回复次数，避免滥用
- **状态持久化**：保存连接状态，支持断线恢复
- **统一API调用**：通过qqApiRequest函数标准化QQ平台API调用
- **智能消息分割**：自动处理超长消息的分段发送
...
**图表来源**
- [index.js:492-500](file://adapters/qq-bot/index.js#L492-L500)
- [index.js:596-635](file://adapters/qq-bot/index.js#L596-L635)
- [index.js:67-84](file://adapters/qq-bot/index.js#L67-L84)
- [sessions.controller.ts:8-18](file://src/sessions/sessions.controller.ts#L8-L18)
...
- [adapter.js:1-35](file://adapters/qq-bot/adapter.js#L1-L35)
- [index.js:67-84](file://adapters/qq-bot/index.js#L67-L84)
- [index.js:368-406](file://adapters/qq-bot/index.js#L368-L406)
- [index.js:95](file://adapters/qq-bot/index.js#L95)
- [index.js:104-114](file://adapters/qq-bot/index.js#L104-L114)
- [index.js:122-131](file://adapters/qq-bot/index.js#L122-L131)
- [index.js:258-301](file://adapters/qq-bot/index.js#L258-L301)
- [index.js:309-353](file://adapters/qq-bot/index.js#L309-L353)
- [index.js:408-406](file://adapters/qq-bot/index.js#L408-L406)
- [index.js:436-549](file://adapters/qq-bot/index.js#L436-L549)
...
**图表来源**
- [index.js:67-84](file://adapters/qq-bot/index.js#L67-L84)
- [index.js:87-89](file://adapters/qq-bot/index.js#L87-L89)
- [index.js:448-477](file://adapters/qq-bot/index.js#L448-L477)
...
**图表来源**
- [index.js:368-406](file://adapters/qq-bot/index.js#L368-L406)
- [index.js:450-462](file://adapters/qq-bot/index.js#L450-L462)
- [index.js:488-491](file://adapters/qq-bot/index.js#L488-L491)
...
### 统一QQ API请求函数
v2.0版本新增了统一的QQ API请求函数qqApiRequest，提供标准化的API调用能力：

- **标准化调用**：统一处理Authorization头、超时控制、错误处理
- **私聊/群聊通用**：支持/users和/groups两种消息发送路径
- **自动鉴权**：每次调用自动获取并使用最新的Access Token
- **超时控制**：10秒超时保护，避免长时间阻塞
- **错误处理**：统一的Promise-based错误处理机制

```mermaid
flowchart TD
Call["调用qqApiRequest"] --> GetToken["获取最新Access Token"]
GetToken --> BuildReq["构建HTTPS请求"]
BuildReq --> SendReq["发送HTTPS请求"]
SendReq --> HandleResp["处理响应"]
HandleResp --> ParseJSON["解析JSON响应"]
...
### 智能长消息分割功能
v2.0版本实现了智能长消息分割功能，解决QQ平台2000字符限制问题：

- **分割策略**：优先在换行处断开，避免在单词中间截断
- **字符限制**：默认2000字符，符合QQ平台限制
- **自动分段**：将超长文本自动分割为多个片段
- **顺序发送**：按顺序发送各片段，避免被限流
- **间隔控制**：多段消息间添加500ms间隔
...
```mermaid
flowchart TD
Input["输入长文本"] --> CheckLen{"长度是否超过2000？"}
CheckLen --> |否| Single["返回单片段"]
CheckLen --> |是| Init["初始化变量"]
Init --> Loop["循环处理剩余文本"]
Loop --> FindCut["寻找最佳断点(换行)"]
FindCut --> CutPoint{"找到断点？"}
CutPoint --> |是| Split["按断点分割"]
CutPoint --> |否| ForceSplit["强制在2000处分割"]
Split --> AddChunk["添加片段到数组"]
ForceSplit --> AddChunk
AddChunk --> MoreText{"还有剩余文本？"}
MoreText --> |是| Loop
MoreText --> |否| ReturnChunks["返回分割后的片段数组"]
```
...
### 改进的会话管理与群聊上下文
v2.0版本改进了会话管理，实现私聊和群聊的精确区分：

- **私聊会话**：使用`c2c-${qqUserId}`作为会话键，确保一对一对话
- **群聊会话**：使用`group-${groupId}-${qqUserId}`作为会话键，实现群内用户隔离
- **上下文隔离**：同一群内的不同用户拥有独立的对话上下文
- **@机器人响应**：群聊中只响应@机器人的消息，过滤普通消息
- **mention过滤**：自动去除@机器人的mention标记，提取真实消息内容
...
RateLimit --> |超过限制| Skip
RateLimit --> |允许处理| FilterMention["过滤@机器人mention"]
FilterMention --> CheckContent{"过滤后内容为空？"}
CheckContent --> |是| Skip
CheckContent --> |否| CreateSession["创建/获取群聊会话"]
CreateSession --> SessionKey["group-${groupId}-${qqUserId}"]
SessionKey --> Chat["调用AI聊天接口"]
Chat --> Reply["发送群聊回复"]
Reply --> Record["记录回复次数"]
```
...
**图表来源**
- [index.js:596-635](file://adapters/qq-bot/index.js#L596-L635)
- [index.js:609](file://adapters/qq-bot/index.js#L609)
...
### 扩展的事件意图覆盖
v2.0版本扩展了事件意图覆盖，支持更多QQ机器人事件类型：

- **C2C_MESSAGE_CREATE**：私聊消息事件，bit 30
- **GROUP_AT_MESSAGE_CREATE**：群内@机器人消息事件，bit 25
- **组合意图**：通过`(1 << 25) | (1 << 30)`同时启用两种事件
- **分片支持**：支持shard参数，便于分布式部署
- **事件过滤**：只处理指定的事件类型，避免不必要的处理

```mermaid
flowchart TD
Intents["事件意图配置"] --> Bit25["GROUP_AT_MESSAGE_CREATE<br/>(bit 25)"]
Intents --> Bit30["C2C_MESSAGE_CREATE<br/>(bit 30)"]
Bit25 --> Combine["组合为 (1<<25)|(1<<30)"]
Bit30 --> Combine
Combine --> Enable["启用事件监听"]
Enable --> Filter["事件过滤处理"]
Filter --> Dispatch["分发到对应处理器"]
```

**图表来源**
- [index.js:470-472](file://adapters/qq-bot/index.js#L470-L472)
- [index.js:492-500](file://adapters/qq-bot/index.js#L492-L500)
...
**图表来源**
- [index.js:436-549](file://adapters/qq-bot/index.js#L436-L549)
- [index.js:507-513](file://adapters/qq-bot/index.js#L507-L513)
- [index.js:563-593](file://adapters/qq-bot/index.js#L563-L593)
- [index.js:596-635](file://adapters/qq-bot/index.js#L596-L635)
...
**图表来源**
- [index.js:408-549](file://adapters/qq-bot/index.js#L408-L549)
- [index.js:67-84](file://adapters/qq-bot/index.js#L67-L84)
- [index.js:368-406](file://adapters/qq-bot/index.js#L368-L406)
- [index.js:450-462](file://adapters/qq-bot/index.js#L450-L462)
...
- [index.js:38-45](file://adapters/qq-bot/index.js#L38-L45)
- [index.js:87-89](file://adapters/qq-bot/index.js#L87-L89)
- [index.js:464-477](file://adapters/qq-bot/index.js#L464-L477)
...
**章节来源**
- [index.js:539-549](file://adapters/qq-bot/index.js#L539-L549)
- [index.js:563-593](file://adapters/qq-bot/index.js#L563-L593)
- [index.js:596-635](file://adapters/qq-bot/index.js#L596-L635)
...
- **会话同步**
  ...
  - 私聊使用qqUserId作为键，群聊使用`group-${groupId}-${qqUserId}`作为键
...
**章节来源**
- [index.js:641-661](file://adapters/qq-bot/index.js#L641-L661)
- [index.js:539-549](file://adapters/qq-bot/index.js#L539-L549)
- [index.js:563-593](file://adapters/qq-bot/index.js#L563-L593)
- [index.js:596-635](file://adapters/qq-bot/index.js#L596-L635)

## 结论
该适配器v2.0版本实现了从简单WebSocket客户端到生产级适配器的重大升级，包含了Access Token鉴权、Op 6 Resume断线恢复、群聊支持、消息去重、频率限制、状态持久化等核心功能。新增的统一QQ API请求函数、智能长消息分割、改进的会话管理和群聊@机器人处理逻辑，进一步提升了适配器的稳定性和用户体验。adapter.js提供了SDK集成的参考路径，index.js则给出了完整的工程化实现。v2.0版本显著提升了适配器的稳定性、可靠性和功能性，为QQ机器人的生产环境部署提供了坚实基础。对于多媒体消息与更复杂的业务场景，可在现有框架上进行扩展，同时注意性能与稳定性保障。

**更新** v2.0版本代表了QQ机器人适配器的重大里程碑，从实验性功能升级为生产级解决方案。

## 附录
- **环境变量示例位置**：.env（用于存放QQ_BOT_APP_ID、QQ_BOT_APP_SECRET、API_BASE、QQ_CHARACTER_ID、QQ_BOT_SANDBOX等）
...
- [index.js:47-58](file://adapters/qq-bot/index.js#L47-L58)