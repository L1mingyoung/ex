...
## 更新摘要
**变更内容**
- 更新Python嵌入服务Dockerfile中的uv sync命令配置，从'uv sync --frozen --no-dev'改为'uv sync --no-dev'
- 修订容器化部署章节，反映依赖管理流程的变更
- 更新依赖分析部分，说明uv.lock文件的作用和依赖锁定机制
...
**更新** 新增Docker容器化部署基础设施，包括Dockerfile、docker-compose.yml和完整的容器化部署指南。**新增QQ Bot服务配置和部署指南**，支持QQ机器人的独立部署与集成。**更新Python嵌入服务依赖管理流程**，采用uv工具进行快速依赖安装。
...
- **uv依赖管理**：Python服务采用uv工具进行快速依赖安装和锁定。
...
- **容器化部署**
  ...
  - Python Dockerfile定义嵌入服务镜像，包含Python运行时、uv工具和依赖锁定。
...
- **uv工具安装**：使用pip安装uv工具，提供快速依赖管理能力
- **依赖安装**：复制pyproject.toml和uv.lock，使用uv sync --no-dev进行依赖安装
- **模型文件**：预拷贝ONNX模型文件，减少运行时下载时间
- **运行时配置**：使用uvicorn启动FastAPI应用，监听8000端口
- **资源优化**：配置并发工作进程，优化CPU利用率

**更新** 依赖管理流程已从'uv sync --frozen --no-dev'更新为'uv sync --no-dev'，这意味着依赖安装将使用最新的兼容版本，而非严格锁定版本。

```mermaid
flowchart TD
Start(["构建Python服务镜像"]) --> Base["使用Python基础镜像"]
Base --> Workdir["设置工作目录 /app"]
Workdir --> InstallUV["pip安装uv工具"]
InstallUV --> CopyManifest["复制pyproject.toml和uv.lock"]
CopyManifest --> SyncDeps["uv sync --no-dev 安装依赖"]
SyncDeps --> CopySrc["复制源代码"]
CopySrc --> ExposePort["暴露8000端口"]
ExposePort --> Run["uv run uvicorn启动服务"]
```
...
- **uv依赖管理**
  - 使用uv工具进行快速依赖安装，通过uv.lock锁定精确版本。
  - 依赖安装命令'uv sync --no-dev'确保生产环境只安装必需依赖。
...
- 嵌入服务依赖
  ...
  - **uv依赖管理**：使用uv.lock锁定精确版本，确保依赖一致性。
- 前端依赖
...
- 嵌入服务性能
  ...
  - **uv依赖管理**：通过uv.lock锁定精确版本，避免依赖升级导致的性能波动。
- 应用层优化
...
- 嵌入服务异常
  ...
  - **uv依赖问题**：如果遇到依赖安装失败，检查uv.lock文件完整性或重新生成依赖锁文件。
- CORS 问题
...
## 结论
通过将后端、嵌入服务与前端解耦部署，结合 Nginx 反向代理、PM2 进程管理与 PostgreSQL + pgvector 的数据库方案，AI Companion 可实现稳定、可扩展的生产级部署。**新增的Docker容器化基础设施进一步提升了部署的一致性和可维护性**。**新增的QQ Bot服务配置为多平台接入提供了完整的解决方案**。**更新的uv依赖管理流程确保了Python服务的依赖一致性与快速部署能力**。建议在上线前完善监控与日志体系、安全加固与备份演练，并制定标准化的 CI/CD 与应急响应流程。
...
- 运维自动化与 CI/CD
  ...
  - **uv依赖管理**：在CI中集成uv.lock文件的生成与验证，确保依赖一致性。
- 微服务治理
...
- 安全最佳实践
  ...
  - **uv依赖安全**：定期更新uv.lock文件，监控依赖安全漏洞，及时修复。