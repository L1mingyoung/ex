...
## 更新摘要
**变更内容**
- 新增 Trae 项目规则文档，包含详细的开发工作记录规则
- 新增前端开发规范章节，涵盖组件、状态管理、样式和命名规范
- 新增后端开发规范章节，引用龙网公司后端代码开发规范
- 更新文档维护规则，明确实施计划和学习笔记的格式要求
- 增强工程规范章节，包含新的开发工作记录要求
...
**更新** 新增 Trae 项目规则文档，明确了开发工作记录的具体要求和格式标准。
...
- **开发工作记录规则**：每次完善或开发工作后，必须将变更内容写入实施计划和学习笔记文档，确保知识传承和可追溯性。
...
**更新** 新增开发工作记录规则，要求每次开发工作后必须更新实施计划和学习笔记文档。

**章节来源**
- [.trae/rules/project_rules.md:3-47](file://.trae/rules/project_rules.md#L3-L47)
...
### 前端开发规范
**更新** 新增前端开发规范章节，明确前端开发的具体要求和最佳实践。

- **组件规范**：使用函数式组件 + Hooks，不使用类组件；组件结构清晰，职责单一
- **状态管理**：使用 Context + useReducer 进行全局状态管理，避免过度使用 Redux
- **样式规范**：使用 CSS 文件，不使用 CSS-in-JS；样式文件与组件文件同目录管理
- **命名规范**：组件使用 PascalCase，CSS 类使用 kebab-case，变量使用 camelCase
- **移动端适配**：使用 CSS 媒体查询，断点为 768px；使用 `env(safe-area-inset-*)` CSS 变量处理安全区域
- **交互反馈**：按钮使用 `:active` 缩放效果，Toast 替代 alert；提供良好的用户反馈
- **性能优化**：合理使用 React.memo，避免不必要的重渲染；组件懒加载

**章节来源**
- [.trae/rules/project_rules.md:49-57](file://.trae/rules/project_rules.md#L49-L57)

### 后端开发规范
**更新** 新增后端开发规范章节，引用龙网公司后端代码开发规范。

- **代码风格**：遵循龙网公司后端代码开发规范（V1.0），包括命名约定、文件组织、注释规范等
- **模块设计**：采用分层架构，清晰的职责分离；服务层负责业务逻辑，控制器层处理 HTTP 请求
- **错误处理**：统一的错误处理机制，提供有意义的错误信息；避免泄露敏感信息
- **安全性**：输入验证、SQL 注入防护、API 密钥管理；遵循最小权限原则
- **性能优化**：数据库查询优化、缓存策略、异步处理；避免 N+1 查询问题
- **测试要求**：单元测试覆盖率要求、集成测试、端到端测试；测试驱动开发

**章节来源**
- [.trae/rules/project_rules.md:59-61](file://.trae/rules/project_rules.md#L59-L61)
...
- **开发工作记录**
  - 每次开发工作完成后，必须更新实施计划和学习笔记文档。
  - 确保记录的完整性和准确性，便于团队知识传承。

**更新** 新增开发工作记录故障排除指南，确保团队成员能够正确维护文档。
...
- [.trae/rules/project_rules.md:5-8](file://.trae/rules/project_rules.md#L5-L8)
- [package.json:8-27](file://package.json#L8-L27)
- [docker-compose.yml:23-35](file://docker-compose.yml#L23-L35)

## 结论
本项目在工程规范、文档维护、容器化与跨语言协作方面建立了清晰的规则与流程。通过严格的变更控制、完善的测试与迁移机制、合理的容器编排与依赖管理，能够有效降低技术债并提升交付质量。

**更新** 新增的 Trae 项目规则进一步强化了开发工作记录制度，确保项目知识的有效传承和团队协作的规范化。

建议团队在日常工作中持续遵循上述规则，并在实践中不断沉淀经验与最佳实践。特别要重视开发工作记录的维护，确保实施计划和学习笔记的及时更新和完整性。
...
- **文档维护**
  - 实施计划格式：YYYY-MM-DD 功能更新：简述，包含变更内容和当前完成状态表格。
  - 学习笔记格式：YYYY-MM-DD | 主题，包含技术要点、踩坑记录、设计决策三个部分。
  - 开发工作记录要求：每次完善或开发工作后必须更新两个文档。

**更新** 新增文档维护章节，明确实施计划和学习笔记的具体格式要求。
...
- [src/app.module.ts:38-50](file://src/app.module.ts#L38-L50)
- [.trae/rules/project_rules.md:10-47](file://.trae/rules/project_rules.md#L10-L47)