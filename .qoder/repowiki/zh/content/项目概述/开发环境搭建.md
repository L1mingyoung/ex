...
- 启动脚本与测试：Windows 批处理脚本提供多终端启动指引，Node 脚本进行端到端测试
...
- 端到端测试失败
  - 确保 Python 嵌入服务与后端均在运行；检查网络连通性与端口占用情况
...
## 结论
通过本指南，开发者可以快速完成 Node.js、TypeScript、Python、PostgreSQL 与 pgvector 的环境搭建，并掌握项目依赖安装、数据库初始化、环境变量配置、Python 虚拟环境设置、开发服务器启动与热重载配置等关键步骤。遇到问题时，可依据故障排除指南进行定位与修复，从而高效开展后续开发工作。
...
- 开发服务器启动
  - 使用批处理脚本在多个终端分别启动数据库、嵌入服务、后端与前端
  - 后端：开发模式支持热重载
  - 前端：Vite 提供热更新与代理
  - 嵌入服务：支持 Mock 模式与真实模型
- 热重载与开发工具
  - 后端：开发模式脚本提供热重载与调试模式
...
- [python/pyproject.toml:1-22](file://python/pyproject.toml#L1-L22)
- [python/scripts/download_model.py:32-38](file://python/scripts/download_model.py#L32-L38)
